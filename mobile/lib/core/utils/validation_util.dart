class ValidationUtil {
  ...
  static bool isValidPassword(String password) {
    // contains at least 8 characters, at least 1 uppercase letter, 1 lowercase letter, 1 number and special character
    return RegExp(r'^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[!@#\$&*~]).{8,}$').hasMatch(password.trim());
  }
  ...
  static bool isValidIdNumber(String idNumber) {
    // valid id number must be 9 or 12 digits
    return RegExp(r'^[0-9]{9}$').hasMatch(idNumber.trim()) || RegExp(r'^[0-9]{12}$').hasMatch(idNumber.trim());
  }
}
