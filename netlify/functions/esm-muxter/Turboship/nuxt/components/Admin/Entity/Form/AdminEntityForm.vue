  ...
  entries.forEach((e) => {
    if (!blackListed.includes(e[0])) {
      let item = {
        name: e[0],
        value: e[1],
        type: globalMeta[props.entityType][e[0]].type,
      }
      fields.push(item)
    }
  })
            ...
            <div v-for="field of items">
              <div class="item">
                <AdminEntityFormField
                  ...
                  :label="globalMeta[entityType][field.name].label"
                  :options="globalMeta[entityType][field.name].options"
                  :multiple="globalMeta[entityType][field.name].type === 'enumeratorMulti'"
                />
                <AdminEntityFormField
                  v-else
                  :help="''"
